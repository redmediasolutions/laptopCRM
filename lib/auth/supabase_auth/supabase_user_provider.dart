...
class LaptopBazarSupabaseUser extends BaseAuthUser {
  LaptopBazarSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> laptopBazarSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LaptopBazarSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
